      ...
      if (mobile) {
        ...
        try {
          if (window.localStorage) {
            isHidden = window.localStorage.getItem('contact-menu-hidden');
          }
        } catch (e) {
    ...
    if (isMobile) {
      try {
        if (typeof window !== 'undefined' && window.localStorage) {
          window.localStorage.setItem('contact-menu-hidden', String(!nextState));
        }
      } catch (e) {
        console.warn('LocalStorage not available', e);
      }
  ...
  return (
    <>
      <div className="fixed bottom-6 right-6 z-[1000] flex flex-col items-center">
        ...
        {!isMobile && (
          <div className="flex flex-col gap-3">
            ...
            >
              ...
              {/* Tooltip */}
              <span
                className="absolute right-full mr-3 whitespace-nowrap text-white text-xs font-semibold px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none bg-slate-900/90 shadow-md"
        ...
        {/* MOBILE VIEW: Expandable Vertical Stack / Collapsed Trigger */}
        {isMobile && (
              ...
              {isOpen && (
                ...
                >
                  ...
                  >
                    ...
                    {/* Blinking Live Badge */}
                    <span className="absolute -top-1 -right-1 px-1.5 py-0.5 bg-red-600 border border-white/20 rounded-full text-[7px] font-black text-white tracking-wider flex items-center gap-0.5 shadow-sm select-none z-10">
                      <span className="w-1.2 h-1.2 rounded-full bg-white animate-live-blink" />
                      LIVE
                    </span>
                  </motion.div>
